    ...
    *_, N_out, _ = Q.shape
    ...
    for block in itertools.product(*(range(s) for s in batch)):
        ...
        for i in range(T_r):
            ...
            last_m_i = m_i
            for j in range(T_c):
                if is_causal and j * B_c > (i+1) * B_r:
                    break
                ...
                if is_causal and j == i:
                    mask = torch.arange(i*T_r, (i+1)*T_r)[:, None] > torch.arange(j*T_c, (j+1) * T_c)[None, :]
                    S_i = torch.where(mask, S_i, neginf)

                print(S_i.shape, m_i.shape)
...
def test_flash_attention_reference(seqlen: int = 32, num_heads: int = 4, headdim: int = 64, dtype = torch.float16, device: str= "cuda:0"):
    device = torch.device(device)
    with device:
        q = torch.randn(seqlen, num_heads, headdim, dtype=dtype)
